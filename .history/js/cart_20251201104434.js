...
function loadCart() {
    ...
    raw.forEach(item => {
        ...
        if (typeof item === "object" && item !== null) {
            const name = item.name || item.title || item.label; // defensive
...
let cart = loadCart();
saveCart(); // ensure storage is normalized on load
...
// Add product to cart (called from product page)
...
// Update the visible cart count (sum of quantities)
...
// Change quantity (delta can be positive or negative)
...
// Checkout via WhatsApp (uses provided number)
function checkoutWhatsApp() {
    ...
    const phone = "2347042424239"; // your number (no plus sign)
    let message = "Hello, I want to order the following products:%0A%0A";

    cart.forEach((item, i) => {
        const name = encodeURIComponent(item.name);
        const qty = Number(item.quantity) || 0;
        message += `${i + 1}. ${name.replace(/%20/g, " ")} (x${qty})%0A`;
    });
